...
This project began as a fork of [glennosss/svgo-autocrop](https://github.com/glennosss/svgo-autocrop), but the core has changed a lot. Cropping now runs in-process with [@resvg/resvg-js](https://github.com/thx/resvg-js), so there is no Puppeteer startup or worker-thread overhead, and batch icon processing is faster. The codebase also moved to TypeScript + SVGO v4. The transform flow is safer too: if any step hits an unsupported case, that step can roll back while the other successful transforms stay in place.

### Quick Start

```terminaloutput
pmpm add -D svgo-autocrop svgo @resvg/resvg-js
```
