...
module.exports = class ImageUtils {
	...
	static async getBounds(imageBytes) {
		...
		// Puppeteer v24+ returns Uint8Array for screenshots, but Jimp 0.16 expects Buffer.
		let imageInput =
			imageBytes instanceof ArrayBuffer ||
			(imageBytes instanceof Uint8Array && !Buffer.isBuffer(imageBytes))
				? Buffer.from(imageBytes)
				: imageBytes;
		let image = await Jimp.read(imageInput);
		...
		image.scan(0, 0, width, height, function (x, y, idx) {
			...
			if (alpha > 0) {
				...
				if (!xMin) {
					// Set all coordinates for first visible pixel
					xMin = xMax = x;
					yMin = yMax = y;
				} else {
					if (x < xMin) {
