...
module.exports = class Ensure {
	...
	static clazz(obj, objDescription = 'class') {
		...
		if (typeof obj.constructor !== 'function') {
			throw Ensure.unexpectedObject(
				'[' + objDescription + "] Expected class, but function doesn't have constructor",
				obj,
			);
		}
		return obj;
	...
	static array(obj, objDescription = 'array') {
		if (!(obj instanceof Array)) {
			// Note: typeof for arrays is 'object'.
			throw Ensure.unexpectedObject(
				'Expected array for ' + objDescription + ", instead '" + typeof obj + "'",
				obj,
			);
		}
		return obj;
