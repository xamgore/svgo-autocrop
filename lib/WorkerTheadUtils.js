	...
	sendResponse(response, shared) {
		...
		let responseCode;
		{
			...
			if (charsRead != json.length) {
				console.error(
					`Buffer not large enough to write full response; json.length=${json.length}, charsRead=${charsRead}, bytesWritten=${bytesWritten}`,
				);
				responseCode = ERROR_BUFFER_TOO_SMALL;
			} else if (bytesWritten <= 0) {
				console.error(
					`Zero or negative number of bytes written; json.length=${json.length}, charsRead=${charsRead}, bytesWritten=${bytesWritten}`,
				);
